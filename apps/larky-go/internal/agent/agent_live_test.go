...
func TestLiveSkillCreatorOutputPath(t *testing.T) {
	...
	// Use a temp dir to simulate a project with .larky/skills/
	workDir := t.TempDir()
	...
	if _, err := os.Stat(expectedFile); os.IsNotExist(err) {
		...
		for _, f := range allFiles {
			if strings.Contains(f, "SKILL.md") && !strings.Contains(f, ".larky/skills/") {
				t.Errorf("SKILL.md created at WRONG location: %s (should be under .larky/skills/)", f)
				wrongLocation = true
			}
		}
		if !wrongLocation {
