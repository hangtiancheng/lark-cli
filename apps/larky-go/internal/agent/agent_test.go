...
func TestSkillCreatorOutputsToCorrectDirectory(t *testing.T) {
	...
	// agent creates the new skill → verify it lands in .larky/skills/, not root
...
New skills MUST be created under the .larky/skills/ directory.
The full path should be .larky/skills/<skill-name>/SKILL.md.
`), 0o644)
	...
	client := &dynamicMock{handlers: []func([]conversation.Message) []llm.StreamEvent{
		...
		// Round 2: user describes, agent creates the skill in .larky/skills/
		func(msgs []conversation.Message) []llm.StreamEvent {
			last := msgs[len(msgs)-1]
...
`,
	...
	// THE KEY CHECK: new skill was created inside .larky/skills/, NOT in project root
...
func TestRealSkillsLoadAndRunSimulation(t *testing.T) {
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .larky/skills directory")
	}
