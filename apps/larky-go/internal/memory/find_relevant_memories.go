...
// FindRelevantMemories. Larky's llm.Client is a streaming + system-prompt-bound interface, so
...
const SelectMemoriesSystemPrompt = `You are selecting memories that will be useful to Larky as it processes a user's query. You will be given the user's query and a list of available memory files with their filenames and descriptions.
...
func selectRelevantMemories(
	...
	// When Larky is actively using a tool (e.g. mcp__X__spawn), surfacing that tool's reference docs
