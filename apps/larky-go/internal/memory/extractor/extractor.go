...
type Deps struct {
	MemoryDir     string                           // <wd>/.larky/memory/ — project/reference (trailing sep)
	UserMemoryDir string                           // ~/.larky/memory/ — user/feedback (trailing sep); may be "" if $HOME unresolved
...
// (Larky messages have no uuid; cursor is the parent conversation's
...
func (e *Extractor) runExtraction(ctx context.Context, isTrailingRun bool) error {
	...
	// Throttle: default is 1 (run every turn). Larky hardcodes 1; trailing runs bypass the throttle
