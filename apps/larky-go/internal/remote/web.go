...
<div id="app">
  <div id="status-bar">
    <span class="brand">⚡ Larky Remote</span>
    <div class="info">
...
function connect() {
  ...
  ws.onopen = () => {
    ...
    // Send application-level ping every 10 seconds to prevent connection being reclaimed by middleware/browser
...
function updateAssistant(el, text, isStreaming) {
  ...
  // Detect <think>...</think> tags to separate thinking content from main body
  ...
  if (thinkMatch) {
    ...
    // Thinking not yet finished, show as collapsed thinking block with cursor
...
function updateToolResult(data) {
  ...
  if (div) {
    ...
    // If no match found, create a completed tool block
...
function renderSlashMenu() {
  ...
  // Scroll to current highlighted item
...
inputEl.addEventListener('input', () => {
  ...
  // Detect if typing a slash command
